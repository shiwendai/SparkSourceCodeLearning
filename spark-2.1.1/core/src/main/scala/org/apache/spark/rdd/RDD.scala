...
abstract class RDD[T: ClassTag](
    @transient private var _sc: SparkContext, // _sc由@transient所以此属性不能被序列化
    @transient private var deps: Seq[Dependency[_]] // 用于存储当前RDD的依赖。RDD的子类在实现时不一定会传递此参数.同样不能被序列化
  ...
  final def dependencies: Seq[Dependency[_]] = {
    ...
    checkpointRDD.map(r => List(new OneToOneDependency(r))).getOrElse {
      if (dependencies_ == null) {
        // 如果dependencies_等于null，那么调用子类实现的getDependencies方法获取当前RDD的依赖后赋予dependencies
   ...
   * RDD is checkpointed or not.
   */
  // partitions方法查找分区数组的优先级为： 从CheckPoint查找 --> 读取partitions_属性 --> 调用getPartitions方法获取
