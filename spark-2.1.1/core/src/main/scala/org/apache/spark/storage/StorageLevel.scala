...
class StorageLevel private(
  ...
  def useDisk: Boolean = _useDisk       // 能否写入磁盘
  def useMemory: Boolean = _useMemory      // 能否写入堆内存
  def useOffHeap: Boolean = _useOffHeap      // 能否写入堆外内存
  def deserialized: Boolean = _deserialized      // 是否需要对Block反序列化
  def replication: Int = _replication      // Block的复制份数
  ...
  // 1111表示存储级别为允许写入磁盘、堆内存及堆外内存，并且需要反序列化
  def toInt: Int = {
    var ret = 0
    if (_useDisk) {
      ret |= 8
  ...
  // 把存储级别和_replication一起被序列化写入外部二进制流
