...
// TaskContext的伴生对象提供了将TaskContext保存到ThreadLocal中，用于保证每个任务尝试线程的TaskContextImpl的线程安全性
object TaskContext {
  /**
   ...
   */
  // 从ThreadLocal中获取当前任务尝试线程的TaskContextImpl，然后调用TaskContextImpl的partitionId方法获取当前Task对应的分区索引
 ...
 * }}}
 */
// TaskContext维护了Task执行时的上下文信息，TaskContext还有一个利用ThreadLocal技术的伴生对象，用于维护每个Task线程的TaskContextImpl.
