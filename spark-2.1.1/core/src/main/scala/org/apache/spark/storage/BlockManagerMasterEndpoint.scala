...
class BlockManagerMasterEndpoint(
  ...
  // Executor ID 与 BlockManagerId之间映射关系的缓存
  private val blockManagerIdByExecutor = new mutable.HashMap[String, BlockManagerId]

  // Mapping from block id to the set of block managers that have the block.
  // BlockId 与存储了此 Block的BlockManager的BlockManagerId之前的一对多关系缓存
  ...
  private def removeBlockFromWorkers(blockId: BlockId) {
    // 从缓存blockLocations中获取BlockId所对应Block实际存储的位置集合locations
    val locations = blockLocations.get(blockId)
    if (locations != null) {
    ...
    if (!blockManagerInfo.contains(id)) {
      blockManagerIdByExecutor.get(id.executorId) match {
        case Some(oldId) =>
          ...
          // 移除blockManagerIdByExecutor、blockManagerInfo、blockLocations等缓存中与此BlockManagerId相关的所有缓存信息
