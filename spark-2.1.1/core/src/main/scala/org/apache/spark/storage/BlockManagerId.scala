...
class BlockManagerId private (
  ...
  // 当前BlockManager所在的实例的ID.如果实例是Driver,那么ID为driver，否则由Master负责给各个Executor分配，
  // ID格式为：app-日期格式字符串-数字
  ...
  // 此端口实际使用了BlockManager中的BlockTransferService对外服务的端口
  def port: Int = port_

  // 拓扑信息
  def topologyInfo: Option[String] = topologyInfo_

  // 判断当前BlockManager所在的实例是否是Driver
