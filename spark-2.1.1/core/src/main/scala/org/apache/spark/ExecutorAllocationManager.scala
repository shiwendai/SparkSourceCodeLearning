 ...
 */
// 准确的说ExecutorAllocationManager是基于工作负载动态分配和删除Executor的代理.
// ExecutorAllocationManager内部会定时根据工作负载计算所需的Executor数量，如果对Executor需求数量大于之前向集群管理器
// 申请的Executor数量，那么向集群管理器申请添加Executor;如果对Executor需求数量小于之前向集群管理器申请的Executor数量，
// 那么向集群管理器申请取消申请部分Executor。此外，ExecutorAllocationManager内部还会定时向集群管理器中申请移除（"杀死"）
// 过期的Executor。
private[spark] class ExecutorAllocationManager(
  ...
  def start(): Unit = {
    ...
    // 创建定时调度的任务scheduleTask，此方法主要调用schedule方法
    ...
    // 将scheduleTask提交给executor(executor是只有一个线程的ScheduleThreadPoollExecutor)，
    // 以固定的间隔intervalMillis(值为100)进行调度
    executor.scheduleWithFixedDelay(scheduleTask, 0, intervalMillis, TimeUnit.MILLISECONDS)

    // 调用ExecutorAllocationClient的requestTotalExecutors方法请求所有的Executor。
    // numExecutorsTarget是动态分配Executor的总数，
    // localityAwareTasks是有本地性偏好的Task数量。
   ...
   */
  // ExecutorAllocationManager的schedule方法，以调整执行Executor请求的数量和运行的Executor的数量
  private def schedule(): Unit = synchronized {
    ...
    // 对过期的Executor进行删除。removeExecutors方法将利用ExecutorAllocationClient的killExecutors方法
    // 通知集群管理器“杀死”Executor. killExecutors方法需要ExecutorAllocationClient的实现类去实现。
   ...
   * 检查我们现有的分配以及我们之前提出的要求是否超出了我们当前的需求。 如果是这样，请截断我们的目标并让集群管理器知道，以便它可以取消不需要的待处理请求。
   * If not, and the add time has expired, see if we can request new executors and refresh the add
   * time.
   * 如果没有，并且添加时间已过期，请查看我们是否可以请求新的执行程序并刷新添加时间。
  ...
  private def updateAndSyncNumExecutorsTarget(now: Long): Int = synchronized {
    ...
    if (initializing) {
      ...
      // 那么首先将numExecutorsTarget设置为maxNeeded与最小Executor数量（minNumExecutors）的最大值
