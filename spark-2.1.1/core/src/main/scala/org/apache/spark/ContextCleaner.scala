...
private[spark] class ContextCleaner(sc: SparkContext) extends Logging {
  ...
  // 用于具体清理工作的线程。此线程为守护线程，名称为SPA入库ContextCleaner
  private val cleaningThread = new Thread() { override def run() { keepCleaning() }}

  // 类型为ScheduledExecutorService，用于执行GC的调度线程池，此线程池只包含一个线程，启动的线程名称以
  // context-cleaner-periodic-gc开头
   ...
   */
    // 执行GC的时间间隔。可以通过spark.cleaner.periodicGC.interval30分钟
   ...
   * resolved.
   */
    // 清理Shuffle数据是否是阻塞式的。默认是false。清理Shuffle数据包括清理MapOutputTracker中指定ShuffleId
    // 对用的map任务状态和ShuffleManager中注册的ShuffleId对应的Shuffle元数据。
  ...
  /** Start the cleaner. */
  def start(): Unit = {
    // 将cleaningThread设置为守护线程，并指定名称为Spark Context Cleaner，然后启动
