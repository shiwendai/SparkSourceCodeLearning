 ...
 * reopened again.
 */
// BlockManager的getDiskWriter方法用于创建DiskBlockObjectWriter。DiskBlockObjectWriter将在shuffle阶段将map任务
// 的输出写入磁盘，这样reduce任务就能够从磁盘中获取map任务的中间输出。

// DiskBlockObjectWriter用于将JVM中的对象直接写入磁盘文件中。DiskBlockObjectWriter允许将数据追加到现有Block。
