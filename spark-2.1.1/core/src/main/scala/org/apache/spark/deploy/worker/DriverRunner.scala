...
private[deploy] class DriverRunner(
  ...
  private[worker] def prepareAndRunDriver(): Int = {
    ...
    // substituteVariables方法的作用是将DriverDescription的command属性的arguments属性中每个参数中的变量替换为
    // 指定的值。例如，将{WORKER_URL}替换为DriverRunner的workerUrl属性
    ...
    // 构造ProcessBuiler,进程命令将由DriverDescription的command属性决定。ProcessBuilder执行Java命令的主类
    // 是DriverWrapper。
