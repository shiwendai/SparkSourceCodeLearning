...
// BlockResult用于封装从本地的BlockManger中获取的Block数据及Block相关联的度量数据。
private[spark] class BlockResult(
    val data: Iterator[Any],  // Block及与Block相关联的度量数据
    val readMethod: DataReadMethod.Value,  // 读取Block的方法。readMethod采用枚举类型的DataReadMethod提供Memory, Disk, Hadoop, Network
...
private[spark] class BlockManager(
  ...
  // 如果部署了外部的Shuffle服务（默认为false），此时将创建ExternalShuffleClient,
  // 但在默认情况下NettyBlockTransferService也会作为Shuffle的客户端。
  ...
  // blockManager刷新driver块位置之前的最大故障数
   ...
   * 使用给定的appId初始化BlockManager。 这不是在构造函数中执行的，因为在BlockManager实例化时可能不知道appId（特别是对于driver，它仅在使用TaskScheduler注册后才知道）。
    *
  ...
  def initialize(appId: String): Unit = {
    ...
    // 生成BlockManager的BlockManagerId
    val id =
      BlockManagerId(executorId, blockTransferService.hostName, blockTransferService.port, None)

    // BlockManager在本地创建的BlockManagerId实际只是在向BlockManagerMaster注册BlockManager时，给BlockManagerMaster
    // 提供参考，BlockManagerMaster将会创建一个包含了拓扑信息的新BlockManagerId作为正式分配给BlockManager的身份标识
    ...
    // 生成shuffleServerId.当启用了外部Shuffle服务时，将新建一个BlockManagerId作为shuffleServerId,
  ...
  private def reportAllBlocks(): Unit = {
    ...
    for ((blockId, info) <- blockInfoManager.entries) {
      // 调用getCurrentBlockStatus获取Block的状态信息BlockStatus
      val status = getCurrentBlockStatus(blockId, info)
      // 如果需要将Block的BlockStatus汇报给BlockManagerMaster,则调用tryToReportBlockStatus方法，
   ...
   */
  // asyncReregister方法实际另起线程调用reregister，来实现异步注册BlockManager
  ...
  override def getBlockData(blockId: BlockId): ManagedBuffer = {
    if (blockId.isShuffle) {
      // 如果当前Block是ShuffleBlock,那么调用shuffleManager.shuffleBlockResolver.getBlockData方法获取Block数据
      shuffleManager.shuffleBlockResolver.getBlockData(blockId.asInstanceOf[ShuffleBlockId])
    } else {
      // 如果不是ShuffleBlock,那么调用getLocalBytes获取Block数据。
      getLocalBytes(blockId) match {
        // 如果调用getLocalBytes获取Block数据。则封装为BlockManagerManagedBuffer
        ...
        case None =>
          ...
          // 否则调用reportBlockStatus方法告诉BlockManagerMaster，此Block不存在
  ...
  def getMatchingBlockIds(filter: BlockId => Boolean): Seq[BlockId] = {
    ...
    // 我们看到除了从BlockInfoManager的entries缓存中获取BlockId外，还需要从DiskBlockManager中获取。
  ...
  private def reportBlockStatus(
    ...
    if (needReregister) {
      // 需要重新向BlockManagerMaster注册当前BlockManager,
      // 因而调用asyncReregister方法向BlockManagerMaster异步注册BlockManager
   ...
   */
  // 向BlockManagerMaster汇报Block的状态信息
  private def tryToReportBlockStatus(
    ...
    // 通过调用BlockManagerMaster的updateBlockInfo方法将向BlockManagerMasterEndpoint发送UpdateBlockInfo消息
   ...
   */
  // 此方法用于从存储体系获取BlockId所对应的Block的数据，并封装为ChunkedByteBuffer后返回
  def getLocalBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    logDebug(s"Getting local block $blockId as bytes")
    // As an optimization for map output fetches, if the block is for a shuffle, return it
    // without acquiring a lock; the disk store never deletes (recent) items so this should work
    if (blockId.isShuffle) {
      // 如果当前Block是ShuffleBlock，那么调用ShuffleManager的shuffleBlockResolver组件的getBlockData方法
      ...
      // 如果当前Block不是ShuffleBlock，那么首先获取Block的读锁，然后调用doGetLocalBytes方法获取Block数据
  ...
  private def doGetLocalBytes(blockId: BlockId, info: BlockInfo): ChunkedByteBuffer = {
    ...
    if (level.deserialized) {
      // Block没有被序列化，按照DiskStore、MemoryStore的顺序获取Block数据
      ...
      if (level.useDisk && diskStore.contains(blockId)) {
        ...
        // 注意：我们故意不要尝试将块放回内存。 由于此分支处理反序列化块，因此该块只能作为对象缓存在内存中，而不是序列化字节。
        // 因为调用者只请求了字节，所以缓存块的反序列化对象没有意义，因为缓存可能没有收益。
      ...
      // 如果Block存储级别说明Block被序列化了，那么按照MemoryStore、DiskStore的顺序，获取Block数据
      if (level.useMemory && memoryStore.contains(blockId)) {
  ...
  private def getLocations(blockId: BlockId): Seq[BlockManagerId] = {
    // 调用BlockManagerMaster的getLocations方法获取所需Block所在的所有位置信息(即BlockManagerId)序列，并随机打乱
    val locs = Random.shuffle(master.getLocations(blockId))
    // 将BlockManagerId序列划分为preferredLocs与otherLocs。preferredLocs位于同一台机器，otherLocs位于不同机器
    val (preferredLocs, otherLocs) = locs.partition { loc => blockManagerId.host == loc.host }
    // 将preferredLocs中的BlockManagerId放置在otherLocs中的BlockManagerId前面
   ...
   */
  // 此方法的作用为从远端的BlockManager以序列化的字节形式获取Block数据。
  def getRemoteBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    ...
    // 调用getLocations方法获取Block所在的所在的所有位置信息序列化locations
    val locations = getLocations(blockId)

    // 设置maxFetchFailures等于locations的大小（即最大获取失败次数）
    val maxFetchFailures = locations.size
    var locationIterator = locations.iterator

    while (locationIterator.hasNext) {
      // 从locations序列中按照顺序取出下一个BlockManagerId
      val loc = locationIterator.next()
      logDebug(s"Getting remote block $blockId from $loc")
      val data = try {
        // 并调用BlockTransferService的fetchBlockSync方法，以同步方式从远端下载Block
        blockTransferService.fetchBlockSync(
          loc.host, loc.port, loc.executorId, blockId.toString).nioByteBuffer()
      } catch {
        // 如果调用fetchBlockSync方法时发生异常，则增加下载失败次数和下载失败总数。
        case NonFatal(e) =>
          runningFailureCount += 1
          totalFailureCount += 1

          // 当totalFailureCount >= maxFetchFailures时，说明已经作了最大努力
          ...
          // 如果存在大量执行程序，则位置列表可能包含大量陈旧条目，导致可能需要大量时间的大量重试.
          // 为了摆脱这些陈旧的条目，我们在一定数量的提取失败后刷新块位置
          if (runningFailureCount >= maxFailuresBeforeLocationRefresh) {
            // 重新调用getLocations方法刷新Block所在的所有位置信息，并将runningFailureCount置为零
      ...
      // 如果以上步骤获取到数据，那么将得到的数据封装为ChunkedByteBuffer并返回
      if (data != null) {
   ...
   */
  // 此方法用于将当前线程持有的Block的写锁降级为读锁，实际是代理了BlockInfoManager.downgradeLock
   ...
   */
  // 此方法用于将任务尝试线程注册到BlockInfoManager
   ...
   *         could not be cached.
   */
  // 此方法用于获取Block,如果Block存在，则获取此Block并返回BlockResult，否则调用makeIterator方法计算Block。
  ...
  def getOrElseUpdate[T](
    ...
    // 调用doPutIterator方法计算、持久化Block。doPutIterator方法的实现与doPutBytes十分相似
    doPutIterator(blockId, makeIterator, level, classTag, keepReadLock = true) match {
      case None =>
        // 返回为None，说明计算得到的Block已经成功存储到内存，因此再次读取此Block
        ...
        Left(blockResult)
      case Some(iter) =>
        // 返回结果为Some，说明计算得到的Block存储到内存时发生了错误
   ...
   */
  // 此方法用于将Block数据写入存储体系，实现步骤和getOrElseUpdate差不多
   ...
   * cases.
   */
  // 此方法用于创建并获取DiskBlockObjectWriter，通过DiskBlockObjectWriter可以跳过对DiskStore的使用，直接将数据写入磁盘
  def getDiskWriter(
      blockId: BlockId,
      file: File,
    ...
    // 属性spark.shuffle.sync将决定DiskBlockObjectWriter把数据写入磁盘时是采用同步还是异步，默认异步
  ...
  def putBytes[T: ClassTag](
    ...
    // putBytes首先对Block数据进行加密处理，然后调用doPutBytes方法写入Block数据。
    val bytesToStore =
  ...
  private def doPutBytes[T](
    ...
    // 首先定义了偏函数，这个偏函数将作为doPut的putBody参数，然后调用doPut方法，doPut方法将调用此偏函数，
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
      val startTimeMs = System.currentTimeMillis
      // Since we're storing bytes, initiate the replication before storing them locally.
      // This is faster as data is already serialized and ready to send.
      val replicationFuture = if (level.replication > 1) {
        // 如果Block的StorageLevel的复制数量大于1，则创建异步线程通过调用replicate方法复制Block数据到其他节点的存储体系
        Future {
          // This is a blocking action and should run in futureExecutionContext which is a cached
          // thread pool
      ...
      if (level.useMemory) {
        ...
        // 如果Block的StorageLevel允许数据写入内存，首先写入内存。
        ...
        // 如果内存不足且Block的StorageLevel允许数据写入磁盘，则写入磁盘
        if (!putSucceeded && level.useDisk) {
          logWarning(s"Persisting block $blockId to disk instead.")
          diskStore.putBytes(blockId, bytes)
        }
      } else if (level.useDisk) {
        // 如果Block的StorageLevel允许数据写入磁盘，则写入磁盘
        diskStore.putBytes(blockId, bytes)
      }
      ...
      if (blockWasSuccessfullyStored) {
        ...
        // 调用reportBlockStatus方法向BlockManagerMaster报告此Block的状态
        info.size = size
   ...
   *                or Some on failure.
   */
  // 函数参数putBody,putBody将执行真正的Block数据写入
  private def doPut[T](
      blockId: BlockId,
      level: StorageLevel,
      classTag: ClassTag[_],
    ...
    val putBlockInfo = {
      ...
      if (blockInfoManager.lockNewBlockForWriting(blockId, newInfo)) {
        newInfo
         ...
         // 如果Block已经存在且不需要持有读锁，则需要当前线程释放持有的读锁。
    ...
    val result: Option[T] = try {
      ...
      if (res.isEmpty) {
        ...
        // 如果写入失败，则调用removeBlockInternal方法移除此Block
      ...
      if (exceptionWasThrown) {
        ...
        // 如果写入异常也需要调用removeBlockInternal方法移除此Block
   ...
   * subsequent reads. This method requires the caller to hold a read lock on the block.
    * 尝试将从磁盘读取的溢出字节缓存到MemoryStore中，以加快后续读取速度。 此方法要求调用者在块上保持读锁定。
   *
   ...
   */
  // 此方法的作用为获取由单个对象组成的Block，实际调用了get方法
   ...
   *         error occurred.
   */
  // 此方法用于将由单个对象组成的Block写入存储体系，实际调用你了putIterator
   ...
   */
  // 此方法用于从内存中删除Block，当Block的存储级别允许写入磁盘，Block将别写入磁盘。此方法主要在内存不足，需要从内存腾出空闲空间时使用
  private[storage] override def dropFromMemory[T: ClassTag](
      blockId: BlockId,
      data: () => Either[Array[T], ChunkedByteBuffer]): StorageLevel = {
    logInfo(s"Dropping block $blockId from memory")

    // 确认当前任务尝试线程是否已经持有对应的写锁
    ...
    if (level.useDisk && !diskStore.contains(blockId)) {
      // Block对应的存储级别允许Block使用磁盘，并且Block尚未写入磁盘，则调用DiskStore的put方法后putBytes方法将Block写入磁盘
    ...
    // Actually drop from memory storememory
    // 如果MemoryStore中存在Block,则调用memoryStore.getSize方法获取将要从内存中删除的Block大小droppedMemorySize
    val droppedMemorySize =
      if (memoryStore.contains(blockId)) memoryStore.getSize(blockId) else 0L

    // 调用memoryStore.remove方法将内存中的Block删除
    ...
    // 获取Block的当前状态
    val status = getCurrentBlockStatus(blockId, info)
    // 调用reportBlockStatus向BlockManagerMaster报告Block状态
    if (info.tellMaster) {
      reportBlockStatus(blockId, status, droppedMemorySize)
    }
    // 当Block写入了磁盘或Block从内存中删除，则调用addUpdatedBlockStatusToTaskMetrics方法更细腻任务度量信息
    if (blockIsUpdated) {
   ...
   */
  // 此方法的作用为移除属于指定Broadcast的所有Block
  ...
  private def removeBlockInternal(blockId: BlockId, tellMaster: Boolean): Unit = {
    ...
    if (tellMaster) {
      // 调用reportBlockStatus向BlockManagerMaster汇报Block状态，
      reportBlockStatus(blockId, BlockStatus.empty)
    }
  }
