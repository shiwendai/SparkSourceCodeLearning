...
private[deploy] class ExecutorRunner(
  ...
  // 此方法核心在于workerThread，workerThread在运行时将执行fetchAndRunExecutor方法
   ...
   */
  // 此方法在杀死CoarseGrainedExecutorBanckend进程后将向Worker发送ExecutorStateChanged消息
  ...
  private def fetchAndRunExecutor() {
    try {
      // Launch the process
      // 构造ProcessBuilder,进程命令将由ApplicationDescription的command属性决定。ApplicationDescription的command属性
      // 何时产生的呢？将在StandaloneSchedulerBackend详细介绍。进程命令的主类是CoarseGrainedExecutorBackend。
      // substituteVariables方法的作用是将ApplicationDescription的command属性的arguments属性中每个参数中的变量替换为指定的值。
      ...
      // 将executorDir设置为ProcessBuilder执行目录。这里的ExecutorDir就是Worker在接收到
      // LaunchExecutor消息后创建的那个executorDir
      ...
      // 启动ProcessBuilder,生成进程。一旦执行了builder.start(),我们利用jdk自带的工具VisualVM,就会发现此时已经产生了一个新的进程
      ...
      // 等待获取进程的退出状态，一旦受到退出状态，则向Worker发送ExecutorStateChanged消息
