 ...
 */
// 通常而言，用户开发的Spark应用程序的提交和执行都离不开 SparkContext 的支持。在正式提交应用之前，首先需要
// 初始化 SparkContext。SparkContext 隐藏了网络通信、分布式部署、消息通信、存储体系、计算引擎、度量系统、
// 文件服务、Web UI 等内容，应用程序开发者只需要使用 SparkContext 提供的API 完成功能开发。
class SparkContext(config: SparkConf) extends Logging {
  ...
  // Spark运行时环境，Executor是处理任务的执行器，它依赖于SparkEnv提供的运行时环境。此外，在Driver中也包含了SparkEnv，
  // 这是为了保证local模式下任务的执行。SparkEnv内部包含了很多组件，例如，SecurityManager、RpcEnv、BlockManager、mapOutTracker等
  ...
  // 心跳接收器。所有Executor都会向HeartbeatReceiver发送心跳信息，HeartbeatReceiver接收到Executor的心跳信息后，首先更新Executor的
  // 最后可见时间，然后将此信息交给TaskScheduler作进一步处理
  ...
  // 在SparkContext的初始化过程中会读取用户指定的Jar包和其他文件
  ...
  try {
    ...
    // 通过addJar和addFile可以将各种任务执行所依赖的文件添加到Driver的RPC环境中，这样各个Executor节点就可以
    // 使用RPC从Driver将问价下载到本地，以供任务执行。
    // Add each JAR given through the constructor
    // 遍历每一个Jar文件并调用addJar方法
    if (jars != null) {
      jars.foreach(addJar)
    }
    // 遍历每一个其他文件并调用addFile方法
    if (files != null) {
    ...
    // 创建和启动调度器(根据不同的maser url创建不同的taskScheduler 和 schedulerBackend)
    val (sched, ts) = SparkContext.createTaskScheduler(this, master, deployMode)
    _schedulerBackend = sched

    // TaskScheduler按照调度算法对集群管理器已经分配给应用程序的资源进行二次调度后分配给任务。taskScheduler
    // 调度的Task是由DAGScheduler创建的，所以DAGScheduler是TaskScheduler的前置调度
    _taskScheduler = ts

    // DAGScheduler是调度系统中的重要组件之一，负责创建Job，将DAG中的RDD划分不同的Stage、提交Stage等。
    ...
    // 当前应用的标识。TaskScheduler启动后会创建应用标识，SparkContext中的_applicationId就是通过
    // 调用taskScheduler.applicationId()获取的
    ...
    // 向事件总线投递SparkListenerApplicationStart(应用启动事件)
    postApplicationStart()

    // Post init 等待SchedulerBackend准备完成
   ...
   */
  // 将调用SparkEnv.RpcEnv.fileServer(fileServer实际上是NettyStreamManager)的addFile方法把Jar文件添加到Driver
  // 本地RpcEnv的NettryStreamManager中，并将Jar文件添加的时间戳信息缓存到addedFiles中
   ...
   */
  // 用于将Jar文件添加到Driver的RPC环境中
  // 将调用SparkEnv.RpcEnv.fileServer(fileServer实际上是NettyStreamManager)的addJar方法把Jar文件添加到Driver
  // 本地RpcEnv的NettryStreamManager中，并将Jar文件添加的时间戳信息缓存到addedJars中
  ...
  private def setupAndStartListenerBus(): Unit = {
    ...
    try {
      // 获取用户自定义的SparkListener的类名。用户可以通过逗号分隔多个自定义Sparklistener
  ...
  private def postEnvironmentUpdate() {
    if (taskScheduler != null) {
      ...
      val environmentDetails = SparkEnv.environmentDetails(conf, schedulingMode, addedJarPaths,
        addedFilePaths)
      // 生成SparkListenerEnvironmentUpdate事件，并投递到总线
