...
private[spark] class ShuffleMapStage(
   ...
   */
  // 当_numAvailableOutputs == numPartitions时为true。也就是说，
  // ShuffleMapStage的所有分区的map任务都执行成功后，ShuffleMapStage才是可用的
  ...
  // 当某一分区的任务执行完成后，首先将分区与MapStatus的对应关系添加到outputLocs中，然后_numAvailableOutputs加一
