...
private[spark] object RpcEnv {
  ...
  def create(
    ...
    // RpcEnvConfig中保存了RpcEnv的配置信息，RpcEnvConfig是一个样例类，用于保存RpcEnv的配置信息
    val config = RpcEnvConfig(conf, name, bindAddress, advertiseAddress, port, securityManager,
      clientMode)
    // 实际创建RpcEnv的动作发生在NettyRpcEnvFactory的create方法中
...
private[spark] abstract class RpcEnv(conf: SparkConf) {
   ...
   */
  // 使用RpcAddress和endpointName，得到对应RpcEndpointRef
