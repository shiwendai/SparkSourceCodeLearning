...
// BlockManagerMaster的作用是对存在于Executor或Driver上的BlockManager进行统一管理。Executor与Driver
// 关于BlockManager的交互都依赖于BlockManagerMaster,比如Executor需要向Driver发送注册BlockManager、
// 更新Executor上Block的最新信息、询问所需要Block目前所在的位置 及 当Executor运行结束需要将此Executor移除等。

// 我们知道Driver上的BlockManagerMaster会实例化并且注册BlockManagerMasterEndpoint.无论是Driver还是Executor,
// 他们的BlockManagerMaster.driverEndpoint属性都将持有BlockManagerMasterEndpoint的RpcEndpointRef.
// 无论是Driver还是Executor，每个BlockManager都拥有自己的BlockManagerSlaveEndpoint.且BlockManager.slaveEndpoint
// 属性保存着各自BlockManagerSlaveEndpoint的RpcEndpointRef.
// BlockManagerMaster负责发送消息，BlockManagerMasterEndpoint负责消息的接收与处理，
// BlockManagerSlaveEndpoint则接收BlockManagerMasterEndpoint下发的命令
private[spark]
class BlockManagerMaster( // BlockManagerMaster负责发送各种与存储体系相关的消息
