...
class ExternalAppendOnlyMap[K, V, C](
   ...
   */
  // ExternalAppendOnlyMap的insertAll方法与ExternalSorter的insertAll方法的实现非常相似。
  // ExternalAppendOnlyMap和ExternalSorter有以下相同点
  //  1.都定义了用于更新和聚合的偏函数update.
  //  2.都使用了AppendOnlyMap的缓存聚合算法
  //  3.都进行了溢出判断和AppendOnlyMap底层data数据溢出到磁盘的操作
  //  4.都调用addElementsRead方法对已经读取的元素数进行统计
  // 不同之处：
  //  1.ExternalAppendOnlyMap使用的是SizeTrackingAppendOnlyMap,
  //    而ExternalSorter使用的是PartitionedAppendOnlyMap
  //  2.ExternalSorter会计算元素的分区ID，而ExternalAppendOnlyMap不会
