...
public class TransportContext {
  ...
  public TransportChannelHandler initializePipeline(
    ...
    try {
    	// 调用createChannelHandler方法创建TransportChannelHandler
      TransportChannelHandler channelHandler = createChannelHandler(channel, channelRpcHandler);

      // 对管道进行设置，这里的ENCODER派生自Netty的Channel OutboundHandler接口；根据Netty的API行为，通过addLast方法注册多个Handler时，
	    // ChannelInboundHandler按照注册的先后顺序执行，ChannelOutBoundHandler按照注册先后顺序逆序执行.
      channel.pipeline()
